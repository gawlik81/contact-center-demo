...
**How to apply:** Przy kolejnych zadaniach DB zakładaj że V001-V089 już istnieją (ostatnia: V089__add_tenant_scoped_retention_indexes.sql, 2026-08-10 — DB-053, 4 indeksy `(tenant_id, kolumna_czasowa)` pod `RetentionPurgeService`/BE-113, zamyka warstwę indeksową EPIC-29 razem z V084's `idx_retention_purge_log_tenant`). Numery migracji kontynuuj od najwyższego istniejącego +1. Zawsze zweryfikuj `ls backend/src/main/resources/db/migration/` przed napisaniem nowej migracji — nie ufaj tej liczbie bezkrytycznie.
