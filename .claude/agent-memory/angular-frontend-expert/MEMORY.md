...
- [WS event architecture dla Agent persony](./project_ws_event_architecture.md) – WS event flow: który serwis obsługuje który typ eventu i dlaczego
- [Attended transfer – frontend flow](./project_attended_transfer_flow.md) – Attended transfer: cykl życia Agent1 i Agent2, kluczowe wzorce guard
- [Design Alignment (maj 2026)](./project_design_alignment.md) – wyniki weryfikacji 13 ekranów agenta względem prototypu JSX, lista zmian CSS/HTML i decyzje zakresu
...
- [WhatsApp manual-connect dialog (2026-08-29)](./project_fe_whatsapp_manual_connect.md) – zastąpiono link zewnętrzny dialogiem z formularzem (phoneNumberId/accessToken/displayName/businessAccountId), signals-only form bez ReactiveFormsModule (pierwszy taki wzorzec w module integrations), `$any($event.target).value` bez importu FormsModule, POST /api/integrations/WHATSAPP/connect; + CR follow-up tego samego dnia: SKIP_ERROR_TOAST (podwójny toast), pierwszy w repo reveal/hide toggle dla tokenu (`.form-field__input-group`/`__input-action`), maxlength 255, `(cancel)` na dialogu dla Escape
