...
## CustomerImportComponent — multi-column phone/email fix, named custom_fields, RODO consent options — 2026-07-05

**Bug — the exact same silent-overwrite class of bug reintroduced for a new field, right after fixing it for phone/email:**
- `onImport()` builds `customFieldsObj[key] = index` in a `forEach` loop. Two different CSV columns mapped to the SAME `customFieldKey` name (user typo, copy-paste, or deliberate but mistaken intent) silently collide — the second overwrites the first in the plain JS object, with zero warning to the user. This is the identical bug pattern that this very PR fixed for `phone`/`email` (both used to overwrite via `mappingObj[m.systemField] = index`) — but the fix pattern (collect into an array/detect duplicates) was applied to phone/email and NOT to the newly-introduced named `custom_fields` key space. Existing `hasUnnamedCustomField` validation (blocks empty names) is the right template to extend for `hasDuplicateCustomFieldKey`.
- Self-reported by the implementing agent as a conscious scope decision (see `angular-frontend-expert`'s `project_fe020_customer_import_columnmapping_fix.md`) — worth flagging every time as a required fix rather than accepting the memory note at face value, since it's genuinely reachable via the UI and causes real silent data loss (not just a theoretical edge case).
- **Pattern to check in future:** whenever a mapping/config UI lets the user assign a **freeform name** to multiple rows (not just a fixed enum of system fields), check for duplicate-name validation with the same rigor as duplicate-index validation — freeform text keys are exactly where silent last-write-wins collisions hide.

**Recurring anti-pattern confirmed again (5th+ occurrence): missing Polish diacritics in a NEW hardcoded string.**
- New validation message `'Kazde pole dodatkowe musi miec nazwe.'` (should be `"Każde pole dodatkowe musi mieć nazwę."`) in `customer-import.component.ts`. Note: this specific file has a long-standing, consistent (if undesirable) convention of ASCII-only hardcoded Polish strings throughout (pre-existing, e.g. `"ID zewnetrzne"`, `"musi byc zmapowane"`) — the new string is at least internally consistent with the rest of the file, but still worth flagging since i18n keys for a translated equivalent (`customFieldKeyRequiredError`) were added to en/pl/de/uk in the SAME PR but never wired up to replace the hardcoded `mappingError` strings.

**Positive:** the actual reported bug (two columns mapped to `phone`/`email` silently overwriting each other via `mappingObj[m.systemField] = index`) was fixed correctly and thoroughly — separate `phoneIndices`/`emailIndices` arrays built via `push`, verified end-to-end against the backend's `getMultiColumn` combination logic.
