...
- **Encapsulation pass dla `CustomerRepository` (domena `customer`):** odłożony — `CustomerRepository` pozostaje `public class` (custom DAO `extends TenantAwareRepository`). Wstrzykiwany bezpośrednio poza `domain.customer` w: `domain/repository/CampaignRepository.java`, `domain/telephony/MockTelephonyAdapter.java`, `domain/telephony/TwilioTelephonyAdapter.java`, `domain/email/EmailContactCreator.java`, `domain/email/TemplateVariableResolver.java`, `domain/service/RoutingService.java`, `domain/service/ContactAssignmentMonitor.java`, `domain/service/GdprService.java`, `api/telephony/TwilioWebhookController.java`, `api/dialer/ManualCallbackController.java`. Zrobić po zakończeniu reorganizacji wszystkich domen, analogicznie do pkt 9 wzorca referencyjnego.
- **Encapsulation pass dla `AppUserRepository` (domena `user`):** odłożony — `AppUserRepository` pozostaje `public interface` (bez zmiany widoczności/logiki w tej sesji). Repozytorium jest wstrzykiwane bezpośrednio w ~20 plikach poza `domain.user` (security/UserDetailsServiceImpl, domain/tenant/TenantServiceImpl, domain/agentgroup/AgentGroupService, domain/service/QueueAssignmentService, domain/service/ScheduledCallbackExecutor, domain/service/CampaignAssignmentService, domain/service/AdminMetricsService, domain/service/ContactService, domain/service/SupervisorMetricsService, domain/service/TransferService, domain/service/ProgressiveDialerService, api/public_/PublicController, domain/service/RoutingService, domain/routing/DefaultRoutingEngine, domain/telephony/TwilioTelephonyAdapter, domain/email/TemplateVariableResolver, domain/agentbreak/AgentBreakActivator i inne). Zrobić po zakończeniu reorganizacji wszystkich domen — analogicznie do wzorca `TenantService.getActiveTenants/getAllTenants/findTenantEntity/updateTenantConfig` z sesji `tenant` (pkt 9 wzorca referencyjnego): dodać do `UserService`/`AdminUserService` brakujące metody odpowiadające wzorcom użycia z zewnątrz, przełączyć `AppUserRepository` na `package-private`, zaktualizować ~20 konsumentów + ich testy.
