...
**How to apply:** Flag all of these patterns in future reviews. For @Transactional + catch(Exception) — always ask whether inner exception can mark outer transaction as rollback-only. For bulk-import features, always check whether dedup/uniqueness lookups are batch-aware or DB-only.
