...
### 12. `TenantContext.isSet()` only tracks `tenantId` — breaks for any legitimately tenant-less authenticated role
Observed in the SUPER_ADMIN role refactor (`rule-refactor` branch, reviewed 2026-07-12): `CrossTenantAspect.verifyTenantContext()` uses `TenantContext.isSet()` (which checks only `TENANT_ID.get() != null`) as its "is this an HTTP request that should have a tenant" signal, then logs `ERROR` if not set and the URI isn't in a small public-path allowlist. `TenantFilter` correctly sets `userId`/`userRole` for SUPER_ADMIN (global admin, `tenant_id IS NULL`) but never sets `tenantId` (by design — SUPER_ADMIN has none). Result: every authenticated SUPER_ADMIN request that reaches a `@Service` in `com.contactcenter.domain` (i.e. almost all of them) trips the `else` branch and logs a false-positive `ERROR "TenantContext NIE JEST ustawiony ... błąd konfiguracji"` — silent (aspect never throws) but pollutes production logs and dilutes the value of a security-monitoring log line the codebase explicitly says is SIEM-watched. Root cause: the aspect's null-tenant detection conflates "no tenant" with "no auth context at all" — the two used to be equivalent before SUPER_ADMIN existed, no longer are. Not fixed as part of this refactor (flagged in CR-BACKEND.md 2026-07-12, unresolved as of that review).

**How to apply:** Whenever a NEW nullable-tenant / cross-tenant / "global" concept is introduced anywhere in this codebase (another global role, a system-level background job, a cross-tenant batch operation, etc.), check every place that currently treats "`TenantContext` unset" as synonymous with "not a real authenticated request" or "misconfiguration" — `CrossTenantAspect` is the known instance, but grep for other consumers of `TenantContext.isSet()`/`getTenantIdOrNull() == null` as a proxy for "something is wrong" before assuming they still hold. The safe fix pattern is to check `TenantContext.getUserRole()` (which IS set for these roles) rather than `isSet()`/tenant presence alone.
