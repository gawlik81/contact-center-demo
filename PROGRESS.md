...
| `campaign` | ✅ | Przeniesiono 13 plików main (Campaign, ScheduledCallback — encje; CampaignRepository, CampaignAssignmentRepository, CampaignContactRepository, ScheduledCallbackRepository — custom DAO `extends TenantAwareRepository`; CampaignService/Impl, CampaignImportService/Impl, CampaignAssignmentService/Impl, ProgressiveDialerService/Impl, DialerCallbackHandler/Impl; CampaignWindowActivator, ScheduledCallbackExecutor — `@Component @Scheduled`). 5 publicznych interfejsów (CampaignService, CampaignImportService — tylko `initiateImport`/`getJobStatus`, CampaignAssignmentService, ProgressiveDialerService, DialerCallbackHandler) + 5 `*Impl` (`package-private`, `@Service`, `@Override` na metodach interfejsu). `ProgressiveDialerService.initiateDialForAgent` musi być w interfejsie — wymagane przez wzorzec `@Lazy` self-proxy (`self.initiateDialForAgent(...)` w `@Transactional`). `CampaignWindowActivator` i `ScheduledCallbackExecutor` przełączone na `package-private` (`@Scheduled`, brak wstrzyknięcia cross-package, analogicznie do `ContactAssignmentMonitor`). Wszystkie 4 repozytoria (`CampaignRepository`, `CampaignAssignmentRepository`, `CampaignContactRepository`, `ScheduledCallbackRepository`) zostają `public` (5-18+ zewnętrznych konsumentów każde — pkt 5 wzorca, w razie wątpliwości `public`), bez encapsulation pass. 4 pliki testowe przeniesione do `domain.campaign` (CampaignImportServiceTest, DialerCallbackHandlerTest, ScheduledCallbackExecutorTest, ProgressiveDialerServiceTest — ten ostatni z `domain.service`), `@InjectMocks`/konstruktory na `*Impl`. Zaktualizowano referencje w 23 plikach (`api/**`, `domain/**`, testy) globalnym `sed` po pełnych ścieżkach `domain.model.{Campaign,ScheduledCallback}`/`domain.repository.{Campaign*,ScheduledCallback}Repository`/`domain.service.{Campaign*,ProgressiveDialerService,DialerCallbackHandler,CampaignWindowActivator,ScheduledCallbackExecutor}` → `domain.campaign.*`. `@EntityScan` w `ContactCenterApplication` rozszerzony o `com.contactcenter.domain.campaign` (repozytoria to custom DAO, `@EnableJpaRepositories` bez zmian). Build: ✅ czysty (`mvn package -pl app -DskipTests`). Testy: 1429, 0 błędów. **Decyzja ws. `dialer`:** `ProgressiveDialerService`/`ProgressiveDialerServiceImpl` i `DialerCallbackHandler`/`DialerCallbackHandlerImpl` semantycznie mogłyby stanowić odrębną domenę `dialer` (silnik wybierania numerów + obsługa dyspozycji połączeń), ale są mocno powiązane z `Campaign`/`CampaignContactRepository`/`ScheduledCallback` (każda metoda operuje na encjach kampanii). Pozostawione w `campaign` dla bezpieczeństwa builda — **kandydat do przyszłej domeny `dialer`**, jeśli kiedyś powstanie odrębny bounded context dla telefonii wychodzącej. |
