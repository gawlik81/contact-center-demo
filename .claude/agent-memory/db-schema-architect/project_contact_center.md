...
**How to apply:** Przy kolejnych zadaniach DB zakładaj że V001-V091 już istnieją na `develop` (V090 = DB-054, ostatni ticket EPIC-29 warstwy DB; V091 = BE-119 funkcja purge_campaign_contact_archive), V092 = social_integration_global_unique_page (zastosowane w bazie dev, plik tylko na gałęzi `feature-socialmedia`), V093 = DB-055 usunięcie duplikatów indeksów `contact` (2026-09-19, patrz [[project_db055_duplicate_contact_indexes]]). Numery migracji kontynuuj od najwyższego istniejącego +1, ALE sprawdź też `flyway_schema_history` na żywej bazie i inne gałęzie — patrz [[feedback_migration_numbering_check]]. Nie ufaj tej liczbie bezkrytycznie.
